...
namespace lamefe
{
    void EncoderImpl::processSub(const std::filesystem::path &a_filename)
    {
        ...
        Win64FileReader l_input(l_inputFilepath);
        WaveReader l_wave(l_input);

        fileLog(a_filename, formatString(l_wave));

        // Create output file
        Win64FileWriter l_output(l_outputFilepath);
        ...
        auto l_data(l_wave.data(0, l_wave.dataSize()));
        auto l_enc(l_output.write(0, unsigned(1.25 * l_sampleNum + 7200)));
        for (uint64_t l_writtenSize = 0; ; )
        {
            const unsigned l_actReadSize =
                            unsigned(l_chunkSize <= l_data.size()? l_chunkSize: l_data.size());
            const unsigned l_readSampleNum = l_actReadSize / l_blockSize;
            ...
            l_writtenSize += l_encSize;
            l_output.flush(l_writtenSize);

            // End the loop if all the wave data is processed
            if (l_readSampleNum <= 0) break;

            l_data.begin += l_actReadSize;
            l_enc.begin += l_encSize;
        }
